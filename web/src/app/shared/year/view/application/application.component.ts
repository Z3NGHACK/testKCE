...
@Component({
    selector: 'shared-view-year-application',
    standalone: true,
    imports: [
        MatIconModule,
        MatTableModule,
...
export class SharedApplicationComponent implements OnInit {
    ...
    view(item: Item): void {
        this.path === 'principal' ? this._router.navigateByUrl(`${this.path}/students/view/${item.id}`) : this._router.navigateByUrl(`/student/view/${item.id}`)
    }
    ...
    listing(_limit: number = 15, _page: number = 1): void {
        ...
        this.isLoading = true;
        this._academicService.listingStudent(this.path ,this.id, param).subscribe({
            next: res => {
                this.data = res.data;
