...
export class UpdateRateComponent {
  ...
  constructor(
      public dialogRef: MatDialogRef<UpdateRateComponent>,
    ...
    this.firstFormGroup = this._formBuilder.group({
      from: ['', [Validators.required, Validators.pattern('^[0-9]+(\.[0-9]{1,2})?$')]], // Allows decimals
      to: ['', [Validators.required, Validators.pattern('^[0-9]+(\.[0-9]{1,2})?$')]],   // Allows decimals
  ...
  onSubmit(): void {
      ...
      this._service.updateNewRate(body, this.id ,this.data.data.id).subscribe({
          next: res => {
            this.data = res;
