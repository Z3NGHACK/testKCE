...
@Component({
    selector: 'principle-view-student-classroom-detail',
    standalone: true,
    imports: [
        MatIconModule,
        PortraitComponent,
...
export class PrincipleClassroomDetailComponent {
    ...
    loading=true;;
    constructor(
        public dialogRef: MatDialogRef<PrincipleClassroomDetailComponent>,
        ...
        if(this.path=='teacher'){
            this.loading=true;
            this._service.datilStudent(this.data.id,this.data.data.id).subscribe((res:any)=>{
            ...
            this.loading=true;
            this._service.datilStudent(this.data.data.id,this.data.id).subscribe((res:any)=>{
                this.attendents=res.attendents;
                this.report=res.report;
                this.dataSource=new MatTableDataSource(this.attendents);
                res.attendents.forEach((a)=>{
    ...
    viewReport(item:any): void {
        ...
        dialogConfig.data = { from: 'teacher',data:item,title:this.student.name,class_id:this.data.id,user_id:this.data.data.id };
        ...
        const dialogRef = this._matDialog.open(
            PrincipleClassroomReportDialogComponent,
            dialogConfig
        );
    ...
    viewDetail(row): void {
        ...
        const dialogRef = this._matDialog.open(
            PrincipleClassroomAttendanceComponent,
            dialogConfig
        );
