...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from 'helper/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[helperScrollbar]',
    exportAs: 'helperScrollbar',
    standalone: true,
})
export class HelperScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_helperScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() helperScrollbar: boolean = true;
    @Input() helperScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('helperScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.helperScrollbar = coerceBooleanProperty(
                changes.helperScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.helperScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('helperScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.helperScrollbarOptions.currentValue
            );
